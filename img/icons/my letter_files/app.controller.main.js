angular.module('AsposeEditorApp').controller('EditorController', function ($scope, $http, $compile) {
		...
		if (initialized === false) {
			...
			if (productName === 'html') {
				...
				if (SourceFormat === 'md') {
					...
					//	buttons = [['viewHTML']].concat(defaultButtons);
					////  'viewHTML': this feature doesn't work and excluded for now
				}
				buttons = [['togglePreview']].concat(buttons);
			}
	...
	$scope.GetEditorHtml = function () {
		...
		// bugfix: make editor to work with zipped html
		if (productName === 'html') {
	...
	$scope.Download = function (outputType) {
		...
		// Attachment sending for email app
		if (productName == "email") {
			...
			$http({
				...
				if (data.StatusCode === 200) {
					var fileLink = fileDownloadLink + "?fileName=" + data.FileName + "&folderName=" + data.FolderName;
			...
			$http({
				method: 'POST',
				url: asposeEditorAPI + 'UpdateContents',
				data: {
					'productName': productName,
					'htmldata': $scope.trum.trumbowyg('html'),
					'folderName': folderName,
